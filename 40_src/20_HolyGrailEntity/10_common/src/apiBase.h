...
public:
	...
	struct meterResult
	{
		...
		// 【この値は概算にすぎない、という申告(2026-08-17)】
		//  測れはしたが、その測り方がこの明るさでは信用できない、と実装が分かっている場合に立てる。
		//  ライブビュー測光は暗くなると数段低く出るが、ヒストグラムは中間調に見えるため
		//  「測れなかった」とも「帯の外」とも判定できない(2026-08-17 に R50V との同時撮影で実証)。
		//  上位はこれが立っていたら、実写を1枚撮ってそのサムネイルで測り直す(初期収束)。
		//  撮影ループ中は関係ない(そちらは APEX で測り方そのものを切り替えている)。
		bool          needShotCheck = false;
	...
	// 測光の間引き(何コマに1回ちゃんと測るか)の状態を捨てる。
	//  間引きは**撮影ループで取得回数の予算を節約するための仕組み**であって、初期収束には
	//  適用してはいけない。初期収束は1枚1枚が高価なので、撮ったら必ず測る必要がある。
	//  これを呼ばずに実写確認を回すと、2枚目で取得した直後にカウンタが立ち、3枚目が
	//  据え置き値を返して1枚まるごと無駄になる(2026-08-17 実機で確認)。
	//  間引きを持たない実装は何もしなくてよい。
	virtual void resetMeterCadence(void) {}
