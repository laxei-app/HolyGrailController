...
namespace
{
	...
	// 測光露出を変えてから、ライブビューがその露出へ追従するまでの待ち[ms]。
	//  これはタイムアウトではなく**無条件の待ち**で、追従したかどうかは見ていない。
	//  ライブビューは30fpsで流れ続けるため、Tvを変えた直後のフレームは
	//  「タイムスタンプは新しいが中身は変更前の露出」になり、readLvHistogram の鮮度判定
	//  (同じフレームを掴んだか)では弾けない。待ちが短ければ古い露出の絵をそのまま採用する。
	// 700 → 2000 (2026-08-17)。R10 がTv変更を反映するのは実測 1.3〜2.1秒
	//  (captureRunner.h の kPrepLeadMs の項に記録)。700ms では構造的に間に合わない。
	// 【どこに効くか】撮影ループでは効かない。撮影中の測光は撮影露出そのもので測るので
	//  乗せ替えが起きず(applied=false)、この待ちを払わない。払うのは
	//  **初期収束(meterHere)で張り付きを抜けるために測光露出をずらしたとき**だけである。
	// 【予算との関係】初期収束は kInitConvergeBudgetMs=25秒 / kInitConvergeTries=8回。
	//  1回の meterHere で最大 kHereMaxShift=4 回ずらすので、最悪 4x2秒=8秒/回になり、
	//  張り付きが続く場面では8回まわる前に予算切れになる(最良推定で撮り始める)。
	//  ずらしが起きるのは張り付いた回だけなので通常は1回以下。
	constexpr int    kHereSettleMs        = 2000;
