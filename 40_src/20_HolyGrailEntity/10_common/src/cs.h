...
namespace hgc
{
	...
	struct videoSet
	{
		...
		// 出来上がりの大きさ: 0=変更なし(保存した画像そのまま=カメラの1/2) / 1=1920x1440 / 2=1920x1080
		uint8_t size   = 0;
	...
	struct cs
	{
		...
		// 同期撮影(2026-08-25)。camera で測光して決めた露出を
		// subCameras の全台へも配り、同じ瞬間に全台のシャッターを切る。
		//
		// 【「パノラマ撮影」から改名(2026-08-29)】用語が誤っていた。パノラマは**視差の
		//  出ない**撮り方を指す言葉で、複数台を同時にシャッターを切ることとは別物
		//  (撮った画像を後で繋いでパノラマにすることはできる)。表示・コメント・識別子・
		//  JSON のキーをすべて syncShot へ揃えた。**古い panorama キーは読まない**
		//  (リリース前なので移行はしない。それ以前の計画では同期撮影が off になる)。
		bool                     syncShot = false;	// 同期撮影を行う
