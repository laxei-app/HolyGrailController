...
namespace hgc
{
	...
	struct cs
	{
		...
		//
		// 【名前が panorama のままなのはなぜか(2026-08-29)】当初「パノラマ撮影」と呼んで
		//  いたが誤り。パノラマは**視差の出ない**撮り方を指す言葉で、複数台を同時に切る
		//  こととは別物(撮った画像を後で繋いでパノラマにすることはできる)。
		//  表示と説明はすべて「同期撮影」へ改めたが、**識別子と JSON のキーは panorama の
		//  まま**にしてある。保存済みの計画ファイルとエッジへ送る JSON がこのキーを使って
		//  おり、変えると既存の計画が読めなくなるため。改名するなら移行の手当てとセットで。
		//  ※ camera は従来どおり「測光担当兼撮影」。panorama=false なら subCameras は無視される。
		//  ※ 機種が違うと設定値の刻みが違うので、適用時に APEX で最寄りへ丸める。
		bool                     panorama = false;	// 同期撮影を行う
