...
public:
	...
	static constexpr int    kCalibMaxShots      = 3;		// 実写確認で撮ってよい上限[枚]
	// 実写確認の時間予算[ms]。kPreConvergeSec=30秒 の中で、ライブビューでの収束(暗所なら
	//  張り付かないので実測5秒程度)を済ませた残りに収める。8秒露光なら2枚(約20秒)まで。
	//  これを超えると撮影窓の開始に食い込み、最初の1〜2コマを落とす。1枚測れれば
	//  サムネイル測光の誤差は0.3段以内なので、確認の3枚目まで粘る価値はない。
	static constexpr int    kCalibBudgetMs      = 20000;
	// 実写確認で撮った1枚のヒストグラム中央値が、この範囲の外なら「上下に張り付いている」。
	//  張り付いていると、ズレは「これ以上」しか分からない(補正しきれない)ので、もう1枚撮って
	//  測り直す。範囲内なら真値が取れているので、確認の1枚は撮らない(サムネイル測光の誤差は
	//  実測0.3段以内で、残るのは 1/3段テーブルへのスナップぶんだけ)。
	static constexpr double kCalibPegHigh       = 0.98;
	static constexpr double kCalibPegLow        = 0.02;
