...
namespace httpAuth
{
	...
	// この相手へ認証付きで叩いても無駄か。無駄なら**そもそも叩かないこと**。
	//
	// 【なぜ要るか(2026-08-28 実機でカメラを3回締め出した)】資格情報が未登録/誤りのまま
	//  撮影の取得ループを回すと、1周で数発の 401 を投げる。60秒周期でも数分で数十発になり、
	//  カメラが 403 "Not access" で締め出す。**本体の接続設定を入れ直すまで戻らない。**
	//  「認証情報を入れ忘れた」だけでカメラが使えなくなるので、割に合わない。
	//  403 を受けた相手は、叩き直しても絶対に回復しない。やめるのが唯一正しい。
	//  資格情報が変われば(setCandidates/addCandidate)自動的に解除される。
	bool blocked(const std::string& host);

	// 403 を受けた。以後この相手へは認証付きで叩かない。
	void noteRefused(const std::string& host);

	// 締め出された相手が1台でもあるか。撮影が始められない理由をユーザーへ伝えるのに使う
	//  (どの相手かまでは要らない。案内は「カメラ本体の設定を入れ直す」の一択なので)。
	bool anyRefused(void);

	// 401 で弾かれた相手が1台でもあるか。「見つからない」と「認証で弾かれた」を
	//  ユーザーへ区別して伝えるために使う。記述子(認証不要)は読めているので、
	//  カメラが居ることは分かっている。通れば noteSuccess が消す。
