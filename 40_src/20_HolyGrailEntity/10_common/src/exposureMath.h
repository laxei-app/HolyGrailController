...
namespace expo
{
	...
	public:
		...
		//  スマホ内蔵カメラは連続に設定できるので、より細かい刻みでテーブルを合成する。
		//  そうすると1歩の大きさがカメラごと(さらに軸ごと)に変わるため、固定値では扱えない。
		//  現在位置の隣へ1目盛り動かしたときの段差を、3軸ぶん見て**いちばん細かいもの**を返す。
		//  上位はこれを「デッドバンドの判定」と「このコマで何目盛り踏めるか」に使う。
