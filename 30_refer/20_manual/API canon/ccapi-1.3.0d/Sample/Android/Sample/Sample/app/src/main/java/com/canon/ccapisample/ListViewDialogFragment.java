...
public class ListViewDialogFragment extends DialogFragment {
    ...
    private final Map<String, EditText> mStringViewMap = new HashMap<>();
    private final Map<String, Spinner> mEnumViewMap = new HashMap<>();
    private final Map<String, EditText> mRangeViewMap = new HashMap<>();
    private final Map<String, Spinner> mIntegerViewMap = new HashMap<>();
        ...
        for(String item : items) {
            ...
            switch (type.get(item)) {
                ...
                case RANGE:
                    ...
                    break;
                case INTEGER:
                    createIntegerLayout(subLayout, item);
                    break;
                default:
                    break;
            }
    ...
    private void createIntegerLayout(LinearLayout layout, String key) {
        Map<String, List<Integer>> ability = mListViewDataSet.getIntegerAbility();
        Map<String, String> current = mListViewDataSet.getValue();
        int index = 0;
        Integer[] list = {0};
        Spinner spinner = new Spinner(getActivity());
        ...
        if( ability.size() != 0 && ability.containsKey(key) ){
            list = ability.get(key).toArray(new Integer[0]);
        }
        else{
            spinner.setEnabled(false);
        }

        ArrayAdapter<Integer> adapter = new ArrayAdapter<>(getActivity(), android.R.layout.simple_spinner_item, list);
        spinner.setAdapter(adapter);

        for(Integer item:list){
            if(item.equals(Integer.parseInt(current.get(key)))){
                spinner.setSelection(index);
                break;
            }
            index++;
        ...
        layout.addView(spinner, new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 2));
        mIntegerViewMap.put(key, spinner);
    }
            ...
            if(mListViewDataSet.isSettable()) {
                ...
                for (String item : items) {
                    switch (type.get(item)) {
                        ...
                        case INTEGER: {
                            Integer value = (Integer) mIntegerViewMap.get(item).getSelectedItem();
                            if (items.size() > 1) {
                                valueSub.put(item, value);
                            } else {
                                body.put(VALUE, value);
