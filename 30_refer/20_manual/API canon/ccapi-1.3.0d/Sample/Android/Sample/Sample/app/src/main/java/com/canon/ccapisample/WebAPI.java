...
import static com.canon.ccapisample.Constants.CCAPI.Field.AUTOPOWEROFF;
import static com.canon.ccapisample.Constants.CCAPI.Field.NO_LIST_OF_APIS;
...
import static com.canon.ccapisample.Constants.CCAPI.Key.FUNCTIONS_CURRENTCONNECTSETTING;
...
import static com.canon.ccapisample.Constants.CCAPI.Key.FUNCTIONS_SENSORCLEANING;
import static com.canon.ccapisample.Constants.CCAPI.Key.SHOOTING_CONTROL_FLICKERDETECTION;
import static com.canon.ccapisample.Constants.CCAPI.Key.SHOOTING_CONTROL_HFFLICKERDETECTION;
import static com.canon.ccapisample.Constants.CCAPI.Key.SHOOTING_CONTROL_HFFLICKERTV;
import static com.canon.ccapisample.Constants.CCAPI.Key.SHOOTING_OPTICALFINDER_AFAREASELECTION;
import static com.canon.ccapisample.Constants.CCAPI.Key.SHOOTING_OPTICALFINDER_AFAREASELECTIONMODE;
import static com.canon.ccapisample.Constants.CCAPI.Key.SHOOTING_OPTICALFINDER_AFFRAMEINFORMATION;
import static com.canon.ccapisample.Constants.CCAPI.Key.SHOOTING_OPTICALFINDER_AFAREAINFORMATION;
...
import static com.canon.ccapisample.Constants.CCAPI.Key.TOPURLFORDEV;
...
class WebAPI{
    ...
    private final Map<String, APIDataSet> mAPIDataMap = new HashMap<>();
    ...
    private boolean isNoListOfApis = false;

    private class SerialThread extends Thread{
        private final ConcurrentLinkedQueue<WebAPIQueueDataSet> mSerialQueue = new ConcurrentLinkedQueue<>();
                ...
                try {
                    if(mSerialQueue.isEmpty() || isWait) {
                        ...
                        if (webAPIQueueDataSet != null) {
                            Log.d(TAG, String.format("%s SerialThread Queue Receive.", webAPIQueueDataSet.getRequestCode()));
    ...
    private class ParallelThread extends Thread{
        private final ConcurrentLinkedQueue<WebAPIQueueDataSet> mParallelQueue = new ConcurrentLinkedQueue<>();
        private boolean isWait = false;
        @Override
        public void run() {
            ...
            while(!mParallelThread.isInterrupted()){
                try {
                    if(mParallelQueue.isEmpty() || isWait) {
                        ...
                        if (webAPIQueueDataSet != null) {
                            Log.d(TAG, String.format("%s ParallelThread Queue Receive.", webAPIQueueDataSet.getRequestCode()));
                            ...
                            mTaskThreadPool.execute(
                                    new Thread() {
                                        @Override
                                        public void run() {
                                            Log.d(TAG, String.format("%s TaskThreadPool Begin.", webAPIQueueDataSet.getRequestCode()));
                                            executeRequest(webAPIQueueDataSet);
                                            Log.d(TAG, String.format("%s TaskThreadPool End.", webAPIQueueDataSet.getRequestCode()));
                                        }
                                    }
                            );
                        }
    ...
    String getUrlForDev(){ return mUrl + "/" + VER100 + "/" + TOPURLFORDEV; }

    boolean isNoListOfApis(String body) {
        JSONObject jsonObject = null;
        try {
            jsonObject = new JSONObject(body);
            //Checks whether a key contains a value.
            if(!jsonObject.isNull(VALUE)) {
                isNoListOfApis = true;
                //Checks whether a string in value matches.
                if (!jsonObject.getString(VALUE).equals(NO_LIST_OF_APIS)) {
                    Log.d(TAG, "Strings do not match:" + jsonObject.toString());
                }
            }
            else{
                isNoListOfApis = false;
            }
        }
        catch (JSONException e) {
            e.printStackTrace();
        }
        return isNoListOfApis;
    }
    ...
    void setNetworkSettingAPIDataList(String body){
        try {
            JSONObject jsonObject = new JSONObject(body);
            for(int j = 0; j < SUPPORTED_API_VERSION.length; j++) {
                if(!jsonObject.isNull(SUPPORTED_API_VERSION[j])) {
                    ...
                    if (jsonArray != null) {
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            if (!apiDataSet.getKey().isEmpty()) {
                                mAPIDataMap.put(apiDataSet.getKey(), apiDataSet);
                                Log.d(TAG, apiDataSet.getKey() + ": " + apiDataSet.getUrl());
                            }
                        }
                    }
                }
    ...
    void enqueueRequest(WebAPIQueueDataSet webAPIQueueDataSet){
        Log.d(TAG, "TaskThreadPool ActiveCount / NumberOfCores :" + mTaskThreadPool.getActiveCount() + "/" + NUMBER_OF_CORES);
    ...
    private void executeRequest(WebAPIQueueDataSet webAPIQueueDataSet){
        switch (webAPIQueueDataSet.getRequestCode()){
            ...
            case GET_FUNCTIONS_NETWORKSETTING:
                sendFunctionNetworksetting(webAPIQueueDataSet, GET);
                break;
            case GET_FUNCTIONS_CURRENTCONNECT_SETTING:
                sendCurrentConnectSetting(webAPIQueueDataSet, GET);
                break;
            case PUT_FUNCTIONS_CURRENTCONNECT_SETTING:
                sendCurrentConnectSetting(webAPIQueueDataSet, PUT);
                break;
            case GET_FUNCTIONS_CONNECT_SETTING:
                sendConnectSetting(webAPIQueueDataSet, GET);
                break;
            case GET_FUNCTIONS_COMM_SETTING:
                sendCommSetting(webAPIQueueDataSet, GET);
                break;
            case GET_FUNCTIONS_FUNCTION_SETTING:
                sendFunctionSetting(webAPIQueueDataSet, GET);
                break;
            case POST_FUNCTIONS_CARDFORMAT:
                sendFunctionsFormat(webAPIQueueDataSet, POST);
                break;
            case POST_FUNCTIONS_SENSORCLEANING:
                sendFunctionsSensorCleaning(webAPIQueueDataSet, POST);
                break;
            ...
            case GET_SHOOTING_CONTROL_IGNORESHOOTINGMODEDIALMODE:
                sendShootingControlIgnoreShootingModedialMode(webAPIQueueDataSet, GET);
                break;
            case POST_SHOOTING_CONTROL_IGNORESHOOTINGMODEDIALMODE:
                sendShootingControlIgnoreShootingModedialMode(webAPIQueueDataSet, POST);
                break;
            ...
            case GET_SHOOTING_SETTINGS_ANTIFLICKERSHOOT:
                sendShootingSettingsAntiFlickerShoot(webAPIQueueDataSet, GET);
                break;
            case PUT_SHOOTING_SETTINGS_ANTIFLICKERSHOOT:
                sendShootingSettingsAntiFlickerShoot(webAPIQueueDataSet, PUT);
                break;
            case GET_SHOOTING_SETTINGS_HFANTIFLICKERSHOOT:
                sendShootingSettingsHFAntiFlickerShoot(webAPIQueueDataSet, GET);
                break;
            case PUT_SHOOTING_SETTINGS_HFANTIFLICKERSHOOT:
                sendShootingSettingsHFAntiFlickerShoot(webAPIQueueDataSet, PUT);
                break;
            case POST_SHOOTING_CONTROL_FLICKERDETECTION:
                sendShootingControlFlickerDetection(webAPIQueueDataSet, POST);
                break;
            case POST_SHOOTING_CONTROL_HFFLICKERDETECTION:
                sendShootingControlHFFlickerDetection(webAPIQueueDataSet, POST);
                break;
            case GET_SHOOTING_SETTINGS_HFFLICKERTV:
                sendShootingSettingsHFFlickerTv(webAPIQueueDataSet, GET);
                break;
            case PUT_SHOOTING_SETTINGS_HFFLICKERTV:
                sendShootingSettingsHFFlickerTv(webAPIQueueDataSet, PUT);
                break;
            case POST_SHOOTING_CONTROL_HFFLICKERTV:
                sendShootingControlHFFlickerTv(webAPIQueueDataSet, POST);
                break;
            ...
            case PUT_SHOOTING_LIVEVIEW_AFFRAMEPOSITION:
                ...
                break;
            case POST_SHOOTING_LIVEVIEW_CLICKWB:
                sendShootingLiveviewClickwb(webAPIQueueDataSet, POST);
                break;
            case GET_SHOOTING_OPTICALFINDER_AFAREASELECTIONMODE:
                sendShootingOpticalfinderAfareaselectionmode(webAPIQueueDataSet, GET);
                break;
            case GET_SHOOTING_OPTICALFINDER_AFAREASELECTION:
                sendShootingOpticalfinderAfareaselection(webAPIQueueDataSet, GET);
                break;
            case PUT_SHOOTING_OPTICALFINDER_AFAREASELECTION:
                sendShootingOpticalfinderAfareaselection(webAPIQueueDataSet, PUT);
                break;
            case GET_SHOOTING_OPTICALFINDER_AFFRAMEINFORMATION:
                sendShootingOpticalfinderAfframeinformation(webAPIQueueDataSet, GET);
                break;
            case GET_SHOOTING_OPTICALFINDER_AFAREAINFORMATION:
                sendShootingOpticalfinderAfareainformation(webAPIQueueDataSet, GET);
                break;
    ...
    private void sendFunctionNetworksetting(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(FUNCTIONS_NETWORKSETTING);
        if(api != null) {
            String body = null;
            if (method.equals(PUT)) {
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendCurrentConnectSetting(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(FUNCTIONS_CURRENTCONNECTSETTING);
        if(api != null) {
            String body = null;
            if (method.equals(PUT)) {
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendConnectSetting(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(FUNCTIONS_CONNECTSETTING);
        if(api != null) {
            sendRequest(method, api.getUrl(), (byte[]) null, webAPIQueueDataSet);
        }
    }

    private void sendCommSetting(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(FUNCTIONS_COMMSETTING);
        if(api != null) {
            sendRequest(method, api.getUrl(), (byte[]) null, webAPIQueueDataSet);
        }
    }

    private void sendFunctionSetting(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(FUNCTIONS_FUNCTIONSETTING);
        if(api != null) {
            sendRequest(method, api.getUrl(), (byte[]) null, webAPIQueueDataSet);
        }
    }
    ...
    private void sendFunctionsSensorCleaning(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(FUNCTIONS_SENSORCLEANING);
        if(api != null) {
            String body = null;
            if (method.equals(POST)) {
                Bundle args = webAPIQueueDataSet.getArguments();
                if (args != null) {
                    body = String.format("{\"%s\": %s}", AUTOPOWEROFF, args.getBoolean(webAPIQueueDataSet.getRequestCode().name()));
                }
            }
            sendRequest(method, api.getUrl(), body, webAPIQueueDataSet);
        }
    ...
    private void sendShootingControlIgnoreShootingModedialMode(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(SHOOTING_CONTROL_IGNORESHOOTINGMODEDIALMODE);
        if(api != null) {
            String body = null;
            if (method.equals(POST)) {
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendShootingSettingsAntiFlickerShoot(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(SHOOTING_SETTINGS_ANTIFLICKERSHOOT);
        if(api != null) {
            String body = null;
            if (method.equals(PUT)) {
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendShootingControlFlickerDetection(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(SHOOTING_CONTROL_FLICKERDETECTION);
        if(api != null) {
            String body = null;
            if (method.equals(POST)) {
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendShootingSettingsHFAntiFlickerShoot(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(SHOOTING_SETTINGS_HFANTIFLICKERSHOOT);
        if(api != null) {
            String body = null;
            if (method.equals(PUT)) {
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendShootingControlHFFlickerDetection(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(SHOOTING_CONTROL_HFFLICKERDETECTION);
        if(api != null) {
            String body = null;
            if (method.equals(POST)) {
                Bundle args = webAPIQueueDataSet.getArguments();
                if (args != null) {
                    ...
                    if(param != null && param.length == 2) {
                        ...
                        try {
                            json.put(ACTION, param[0]);
                            json.put("apply_result", Boolean.valueOf(param[1]));
                        }
                        catch (JSONException e) {
                            e.printStackTrace();
                        }
    ...
    private void sendShootingSettingsHFFlickerTv(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(SHOOTING_SETTINGS_HFFLICKERTV);
        if(api != null) {
            String body = null;
            if (method.equals(PUT)) {
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendShootingControlHFFlickerTv(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(SHOOTING_CONTROL_HFFLICKERTV);
        if(api != null) {
            String body = null;
            if (method.equals(POST)) {
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendShootingLiveviewClickwb(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        APIDataSet api = mAPIDataMap.get(SHOOTING_LIVEVIEW_CLICKWB);
        if(api != null) {
            String body = null;
            if (method.equals(POST)) {
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendShootingOpticalfinderAfareaselectionmode(WebAPIQueueDataSet webAPIQueueDataSet, String method) {
        APIDataSet api = mAPIDataMap.get(SHOOTING_OPTICALFINDER_AFAREASELECTIONMODE);
        if (api != null) {
            sendRequest(method, api.getUrl(), (byte[]) null, webAPIQueueDataSet);
        }
    }

    private void sendShootingOpticalfinderAfareaselection(WebAPIQueueDataSet webAPIQueueDataSet, String method) {
        APIDataSet api = mAPIDataMap.get(SHOOTING_OPTICALFINDER_AFAREASELECTION);
        if (api != null) {
            if (method.equals(PUT)) {
                String body = null;
                Bundle args = webAPIQueueDataSet.getArguments();
    ...
    private void sendShootingOpticalfinderAfframeinformation(WebAPIQueueDataSet webAPIQueueDataSet, String method) {
        APIDataSet api = mAPIDataMap.get(SHOOTING_OPTICALFINDER_AFFRAMEINFORMATION);
        if (api != null) {
            sendRequest(method, api.getUrl(), (byte[]) null, webAPIQueueDataSet);
        }
    }

    private void sendShootingOpticalfinderAfareainformation(WebAPIQueueDataSet webAPIQueueDataSet, String method) {
        APIDataSet api = mAPIDataMap.get(SHOOTING_OPTICALFINDER_AFAREAINFORMATION);
        if (api != null) {
            sendRequest(method, api.getUrl(), (byte[]) null, webAPIQueueDataSet);
        }
    }
    ...
    private void sendEventPolling(WebAPIQueueDataSet webAPIQueueDataSet, String method){
        ...
        if(api != null) {
            ...
            Bundle args = webAPIQueueDataSet.getArguments();
            if (args != null) {
                String query = args.getString(webAPIQueueDataSet.getRequestCode().name());
                if(query != null){
                    url += ("?" + query);
                }
            }
