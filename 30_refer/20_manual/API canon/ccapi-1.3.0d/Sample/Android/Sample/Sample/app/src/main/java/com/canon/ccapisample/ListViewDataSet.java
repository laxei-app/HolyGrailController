...
class ListViewDataSet implements Serializable{
    ...
    private final Map<String, List<String>> mEnumAbility = new HashMap<>();
    private final Map<String, Map<String, Integer>> mRangeAbility = new HashMap<>();
    private final Map<String, List<Integer>> mIntegerAbility = new HashMap<>();
    private List<String> mItems = new ArrayList<>();
    private final Map<String, DataType> mDataType = new HashMap<>();
    ...
    private void parseArrayAbility(String name, JSONArray jsonArrayAbility)  throws JSONException{
        if (jsonArrayAbility.length() > 0) {
            ...
            else if (jsonArrayAbility.get(0) instanceof Integer) {
                List<Integer> list = new ArrayList<>();
                for (int i = 0; i < jsonArrayAbility.length(); i++) {
                    list.add(jsonArrayAbility.getInt(i));
                }
                mIntegerAbility.put(name, list);
                mDataType.put(name, DataType.INTEGER);
            }
            else {
    ...
    Map<String, List<Integer>> getIntegerAbility() {
        return mIntegerAbility;
    }
    ...
    boolean isSettable(){
        ...
        if(mItems.size() != 0){
            ...
            if(mEnumAbility.size() != 0 || mRangeAbility.size() != 0 || mIntegerAbility.size() != 0) {
