...
public class SubActivity extends AppCompatActivity implements EventListener, AuthenticateListener, DisconnectListener{
    ...
    static final String IS_USE_IPV6 = "isUseIPv6";

    private boolean mIsUseIPv6 = false;
    ...
    private final Map<String, Object> mSettingsInfoMap = new HashMap<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setIsUseIPv6( (boolean) intent.getSerializableExtra(IS_USE_IPV6));
        ...
        if (savedInstanceState == null) {
            ...
            switch (mCurrentScreen) {
                case RemoteCapture:
                    transaction.replace(R.id.container, RemoteCaptureFragment.newInstance(mSettingsInfoMap.get(SHOOTING_SETTINGS).toString(), getIsUseIPv6()));
                    break;
                ...
                case ContentsViewer:
                    ...
                    break;
                case NetworkSetting:
                    transaction.replace(R.id.container, NetworkSettingFragment.newInstance());
                    break;
            }
            transaction.commit();
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        else if (id == R.id.action_network_settings && mCurrentScreen != Screen.NetworkSetting) {
            APIDataSet api = WebAPI.getInstance().getAPIData(FUNCTIONS_NETWORKSETTING);
            if(api != null){
                mCurrentScreen = Screen.NetworkSetting;
                transaction.replace(R.id.container, NetworkSettingFragment.newInstance());
                transaction.commit();
            }else{
                Toast.makeText(this, "An incompatible model is connected.", Toast.LENGTH_SHORT).show();
            }
        }
    ...
    private String readFile(String filename) throws IOException{
        ...
        try (InputStream input = this.getAssets().open(filename)) {
    ...
    private void setIsUseIPv6(boolean isUseIpv6){
        mIsUseIPv6 = isUseIpv6;
    }
    private boolean getIsUseIPv6(){
        return mIsUseIPv6;
    }
}
